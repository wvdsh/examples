...
- a Rust-owned Wavedash startup flow via imported browser host bindings
...
- `Rust`: exports the raw wasm entrypoints `wd_init`, `wd_resize`, and `wd_tick`.
- `Rust`: owns the Wavedash startup phases, game state, AI, scoring, collision, and rendering commands.
- `Rust`: calls imported host functions for canvas drawing and SDK access.
...
- `JavaScript`: forwards Wavedash SDK calls into Rust without owning gameplay rules.
...
2. Run `rustup target install wasm32-unknown-unknown`.
3. Run `./build-web.sh`.

This builds a raw `cdylib` wasm module at `build/web/game.wasm` and copies the host entrypoint to `build/web/game.js`.
