...
`example-cpp` is a minimal C++ + WebAssembly pong demo that uses the Wavedash custom-engine flow.
...
- `src/main.cpp`: pong game state, AI, physics, drawing calls, and the C++-owned Wavedash startup state machine
...
The source is freestanding C++17 with no standard library. It uses language features that have zero runtime cost:

- `constexpr` for compile-time constants instead of `#define` macros
- `enum class` for type-safe `GameMode` and `StartupPhase` enumerations
- function templates for generic `clamp`, `abs_val`, and `min_val` utilities
- a `StringView` struct with a compile-time `sv()` template that deduces literal lengths
- default member initializers on `State` so a reset is just `state = State{}`
- `extern "C"` blocks for the wasm import/export boundary
- `static_cast` instead of C-style casts
- `auto` for type deduction in local variables
