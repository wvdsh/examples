...
async function save() {
  ...
  try {
    ...
    const wrote = await WavedashJS.writeLocalFile(SAVE_PATH, bytes);
    ...
    const uploadResponse = await WavedashJS.uploadRemoteFile(SAVE_PATH);
...
async function load() {
  ...
  try {
    const downloadResponse = await WavedashJS.downloadRemoteFile(SAVE_PATH);
    if (!downloadResponse || !downloadResponse.success) {
      throw new Error(downloadResponse?.error || "downloadRemoteFile failed");
    }
    const bytes = await WavedashJS.readLocalFile(SAVE_PATH);
...
async function deleteSave() {
  ...
  try {
    const deleteResponse = await WavedashJS.deleteRemoteFile(SAVE_PATH);
...
async function refreshSaveMeta() {
  try {
    ...
    const listResponse = await WavedashJS.listRemoteDirectory(parent);
...
async function unpublish() {
  ...
  try {
    const response = await WavedashJS.deleteUGCItem(publishedUgcId);
...
async function importScene() {
  ...
  try {
    ...
    const response = await WavedashJS.downloadUGCItem(id, IMPORT_SCRATCH_PATH);
    if (!response || !response.success) throw new Error(response?.error || "downloadUGCItem failed");
    const bytes = await WavedashJS.readLocalFile(IMPORT_SCRATCH_PATH);
    if (!bytes) throw new Error("readLocalFile returned null");
    const count = applySceneFromBytes(bytes);
