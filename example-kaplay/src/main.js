...
// Sim runs in "world units" identical to the Three.js example so physics
// feels the same; we scale to pixels for rendering.
const SCALE = 100;
const FIELD_W = 16;
const FIELD_H = 9;
const PADDLE_W = 0.35;
...
const CANVAS_W = FIELD_W * SCALE;
const CANVAS_H = FIELD_H * SCALE;
...
function worldToScreen(x, y) {
  return k.vec2(CANVAS_W / 2 + x * SCALE, CANVAS_H / 2 - y * SCALE);
}

const centerLine = k.add([
  k.rect(0.08 * SCALE, FIELD_H * SCALE),
...
const leftPaddle = k.add([
  k.rect(PADDLE_W * SCALE, PADDLE_H * SCALE),
  k.pos(worldToScreen(LEFT_X, 0)),
  k.anchor("center"),
  k.color(59, 130, 246),
]);

const rightPaddle = k.add([
  k.rect(PADDLE_W * SCALE, PADDLE_H * SCALE),
  k.pos(worldToScreen(RIGHT_X, 0)),
  k.anchor("center"),
  k.color(239, 68, 68),
]);

const ball = k.add([
  k.circle((BALL_SIZE / 2) * SCALE),
  k.pos(worldToScreen(0, 0)),
  k.anchor("center"),
  k.color(241, 245, 249),
]);
...
function focusCanvas() {
  // Kaplay binds key listeners to the canvas, so HTML buttons stealing focus
  // would swallow arrow/WASD input. Always hand focus back after a DOM click.
  canvas.focus();
}
...
function setGameplayVisible(show) {
  leftPaddle.hidden = !show;
  rightPaddle.hidden = !show;
  ball.hidden = !show;
  centerLine.hidden = !show;
...
function pollInput() {
  input.leftUp = k.isKeyDown("w");
  input.leftDown = k.isKeyDown("s");
  input.rightUp = k.isKeyDown("up");
  input.rightDown = k.isKeyDown("down");
}

k.onKeyPress("escape", onEscape);
...
function enterLocalGame() {
  ...
  leftName = Wavedash.getUsername() || "Player";
  rightName = "Guest";
  serveDir = 1;
  resetBall();
  updateScoreboard();
...
function enterLobby(hostRole) {
  ...
  peerConnected = false;
  leftScore = 0; rightScore = 0;
  const self = Wavedash.getUsername() || "Player";
  if (isHost) {
    leftName = self;
    rightName = "Waiting...";
    lobbyStatusEl.textContent = "Share this lobby with a friend";
  } else {
    leftName = (peerId && Wavedash.getUsername(peerId)) || "Host";
    rightName = self;
...
async function refreshLobbies() {
  const response = await Wavedash.listAvailableLobbies();
...
function renderLobbyList(lobbies) {
  ...
  for (const lobby of lobbies) {
    ...
    if (!id) continue;
    const hostUsername = Wavedash.getLobbyData(id, "host_username");
...
async function tryJoinLobby(lobbyId) {
  const response = await Wavedash.joinLobby(lobbyId);
...
async function onCreateLobby() {
  const maxPlayers = 2;
  const response = await Wavedash.createLobby(LOBBY_VISIBILITY_PUBLIC, maxPlayers);
  if (!response || !response.success) return;
  const lobbyId = response.data;
  const username = Wavedash.getUsername();
  if (username) Wavedash.setLobbyData(lobbyId, "host_username", username);
...
async function leaveCurrentLobby() {
  ...
  peerId = "";
  peerConnected = false;
  if (id) await Wavedash.leaveLobby(id);
  enterOnlineMenu();
}
...
Wavedash.addEventListener(Wavedash.Events.LOBBY_JOINED, (e) => {
  const payload = e.detail;
  currentLobbyId = payload.lobbyId;
  const selfId = Wavedash.getUserId();
...
Wavedash.addEventListener(Wavedash.Events.LOBBY_USERS_UPDATED, (e) => {
  if (state !== State.IN_LOBBY && state !== State.ONLINE_GAME) return;
  const { userId, changeType } = e.detail;
  if (userId === Wavedash.getUserId()) return;
...
Wavedash.addEventListener(Wavedash.Events.P2P_CONNECTION_ESTABLISHED, (e) => {
  if (state !== State.IN_LOBBY) return;
  const { userId, username } = e.detail;
  if (!peerId) peerId = userId;
  else if (userId !== peerId) return;
  peerConnected = true;
  const peerUsername = username || Wavedash.getUsername(peerId) || "Guest";
  if (isHost) {
...
Wavedash.addEventListener(Wavedash.Events.P2P_PEER_DISCONNECTED, (e) => {
  if (state !== State.IN_LOBBY && state !== State.ONLINE_GAME) return;
  if (e.detail.userId !== peerId) return;
  leaveCurrentLobby();
});

Wavedash.addEventListener(Wavedash.Events.LOBBY_KICKED, () => {
  currentLobbyId = "";
  peerId = "";
  peerConnected = false;
  enterOnlineMenu();
...
function sendJSON(channel, reliable, obj) {
  ...
  Wavedash.broadcastP2PMessage(channel, reliable, bytes);
}
...
function drainChannel(channel) {
  const out = [];
  while (true) {
    const msg = Wavedash.readP2PMessageFromChannel(channel);
    if (!msg) break;
    try {
...
const launchLobbyId = Wavedash.getLaunchParams().lobby;
if (launchLobbyId) {
  (async () => {
    const response = await Wavedash.joinLobby(launchLobbyId);
