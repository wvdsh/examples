...
let serving = true;
let serveDir: 1 | -1 = Math.random() < 0.5 ? 1 : -1;
...
const input: Input = { up: false, down: false };
...
function toX(gx: number): number { return innerWidth / 2 + gx * ppu(); }
function toY(gy: number): number { return innerHeight / 2 - gy * ppu(); }

function resetBall(dir: 1 | -1): void {
  ballX = 0; ballY = 0;
