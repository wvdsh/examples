...
${ev.error?.stack ?? "(no stack available)"}`;
  ...
  var meta = [
    {"name":"Pong","author":"Wavedash","site":"https://wavedash.com","version":"1.0.0"}
  ][0];
...
rooms.templates['rm_main'] = {
    ...
    onStep() {
        /* 🐱👉 room rm_main — On frame start event (core_OnStep) */
{
const r = this ;
const dt = u.time;

const up = (inputs ).registry['keyboard.KeyW'] || (inputs ).registry['keyboard.ArrowUp'];
const down = (inputs ).registry['keyboard.KeyS'] || (inputs ).registry['keyboard.ArrowDown'];
...
r.label.text = r.playerScore + '  |  ' + r.cpuScore;
    ...
    onCreate() {
        /* 🐱👉 room rm_main — On room start event (core_OnRoomStart) */
{
const r = this ;
r.fieldW = 960;
r.fieldH = 540;
