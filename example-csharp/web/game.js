(function () {
  // JS owns browser plumbing only. C# owns startup sequencing and gameplay.
  ...
  function createShell(target) {
    ...
    title.textContent = "Booting example-csharp";
    ...
    detail.textContent = "Load stepping, SDK init, then a C# Pong demo.";
  ...
  function showFatal(shell, message, error) {
    ...
    shell.overlayTitle.textContent = "Failed to boot example-csharp";
  ...
  async function main() {
    ...
    await runStep(
      shell,
      sdk,
      "Fetching C# WASM module",
      0.18,
      "Downloading the C# game binary before handing startup control to WebAssembly.",
      async () => {
    ...
    await runStep(
      ...
      0.3,
      "Creating the C# module and binding browser host functions.",
      async () => {
    ...
    if (!memory || typeof wd_init !== "function" || typeof wd_resize !== "function" || typeof wd_tick !== "function") {
      throw new Error("The C# module must export memory, wd_init, wd_resize, and wd_tick.");
    }
