(function () {
  // JS owns browser plumbing only. C# (MonoGame) owns startup sequencing and
  // gameplay via the Game1 class and its Initialize/Update/Draw lifecycle.
  const BOOT_STEP_DELAY_MS = 70;
  const SDK_EVENTS = {
  ...
  function createShell(target) {
    ...
    title.textContent = "Booting example-monogame";
    ...
    detail.textContent = "Load stepping, SDK init, then a MonoGame Pong demo.";
  ...
  async function runStep(shell, sdk, label, progress, detail, task) {
    ...
    await sleep(BOOT_STEP_DELAY_MS);
    return task();
  }
  ...
  function showFatal(shell, message, error) {
    ...
    shell.overlayTitle.textContent = "Failed to boot example-monogame";
  ...
  async function main() {
    ...
    function callWithHostError(label, fallbackValue, task) {
      try {
        return task();
      } catch (error) {
        captureHostError(label, error);
        return fallbackValue;
      }
    }

    function callMaybeAsyncWithHostError(label, task) {
      try {
    ...
    function readWasmText(ptr, len) {
      return decodeWasmString(wasmInstance, textDecoder, ptr, len);
    }

    function writeWasmText(ptr, maxLen, value) {
      return writeWasmString(wasmInstance, textEncoder, ptr, maxLen, value);
    }
    ...
    await runStep(
      shell,
      sdk,
      "Fetching MonoGame WASM module",
      0.18,
      "Downloading the MonoGame game binary before handing startup control to WebAssembly.",
      async () => {
    ...
    const imports = {
      env: {
        ...
        js_draw_text(ptr, len, x, y, size, r, g, b, a) {
          const text = readWasmText(ptr, len);
          const ctx = renderer.ctx;
          ctx.fillStyle = rgba(r, g, b, a);
        ...
        js_host_set_loading(stepPtr, stepLen, detailPtr, detailLen, progress) {
          updateLoading(
            shell,
            readWasmText(stepPtr, stepLen),
            progress,
            readWasmText(detailPtr, detailLen)
          );
        },
        js_host_set_status(ptr, len, r, g, b, a) {
          setStatus(shell, readWasmText(ptr, len), rgba(r, g, b, a));
        },
        js_host_set_user(ptr, len) {
          setUser(shell, readWasmText(ptr, len));
        },
        ...
        js_host_show_fatal(messagePtr, messageLen, detailPtr, detailLen) {
          showFatal(shell, readWasmText(messagePtr, messageLen), readWasmText(detailPtr, detailLen));
        },
        js_host_has_error() {
          return hostError ? 1 : 0;
        },
        js_host_write_error(ptr, maxLen) {
          return writeWasmText(ptr, maxLen, hostError);
        },
        js_wd_init(debug, deferEvents) {
          callMaybeAsyncWithHostError("WavedashJS.init failed", () =>
            sdk.init({
              debug: Boolean(debug),
        ...
        js_wd_is_ready() {
          return callWithHostError("WavedashJS.isReady failed", 0, () =>
            typeof sdk.isReady !== "function" || sdk.isReady() ? 1 : 0
          );
        },
        js_wd_update_load_progress(progress) {
          callWithHostError("WavedashJS.updateLoadProgressZeroToOne failed", undefined, () => {
            if (typeof sdk.updateLoadProgressZeroToOne === "function") {
              sdk.updateLoadProgressZeroToOne(progress);
            }
            return undefined;
    ...
    await runStep(
      ...
      0.3,
      "Creating the MonoGame module and binding browser host functions.",
      async () => {
        wasmInstance = await instantiateWasm("./game.wasm", imports);
      }
    );

    // NativeAOT WASM modules may export _initialize for runtime setup (GC,
    // static constructors). Call it before any game exports if present.
    if (typeof wasmInstance.exports._initialize === "function") {
      wasmInstance.exports._initialize();
    }

    const { memory, wd_init, wd_resize, wd_tick } = wasmInstance.exports;
    if (!memory || typeof wd_init !== "function" || typeof wd_resize !== "function" || typeof wd_tick !== "function") {
      throw new Error("The MonoGame module must export memory, wd_init, wd_resize, and wd_tick.");
    }

    let lastFrame = performance.now();

    wd_init(renderer.width, renderer.height);

    window.addEventListener("resize", () => {
      renderer.resize();
      wd_resize(renderer.width, renderer.height);
    });
